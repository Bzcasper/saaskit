  ...
  return (
    <div class={CARD_STYLES}>
      <div class="flex-1 space-y-4">
        <div>
          <h2 class="font-heading font-bold text-h3 text-foreground mb-2">
            Free
          </h2>
          <p class="text-foreground-muted text-body">
            Perfect for getting started with music trend discovery.
          </p>
        </div>
        <p class="py-4">
          <span class="font-heading font-black text-h1 text-foreground">
            Free
          </span>
        </p>
        <div class="space-y-3">
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Browse trending music
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Basic search & discovery
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Community participation
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Standard analytics
          </p>
        </div>
      </div>
      ...
      <div class="flex-1 space-y-4 pt-4">
        <div>
          <div class="flex items-center gap-3 mb-2">
            <h2 class="font-heading font-bold text-h3 text-foreground">
              {product.name}
            </h2>
        ...
        <p class="py-4">
          ...
          <span class="text-foreground-muted text-body">
            / {product.default_price.recurring?.interval}
          </span>
        </p>
        <div class="space-y-3">
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Everything in Free
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Advanced trend analytics
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> AI-powered recommendations
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Premium user badge
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Priority API access
          </p>
        </div>
      </div>
      <div class="text-center pt-6 mt-6 border-t border-border">
  ...
  return (
    <div class={CARD_STYLES}>
      <div class="flex-1 space-y-4">
        ...
        <div class="space-y-3">
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Everything in Premium
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Custom API limits
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Dedicated support
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> SLA guarantee
          </p>
          <p class={FEATURE_STYLES}>
            <IconCheckCircle class={CHECK_STYLES} /> Custom integrations
          </p>
        </div>
      </div>
      <div class="text-center pt-6 mt-6 border-t border-border">
        <a
          href="mailto:contact@trendradar.io"
...
export default defineRoute<State>(async (_req, ctx) => {
  ...
  if (data.length === 0) {
    throw new Error(
      "No Stripe products found. Please run 'deno task init:stripe' first.",
    );
  }
                ...
                <div class="w-12 h-12 mx-auto mb-2 rounded-full bg-primary/10 flex items-center justify-center">
                  ...
                  >
                    <path
                      ...
                      d="M12 22s8-4 8-4 8 8 8 4-8 8-4-4-4-4m4-4h6m-6 0h6m-6 0v6m0-6v6"
                    />
                  </svg>
                </div>
                ...
                <div class="w-12 h-12 mx-auto mb-2 rounded-full bg-success/10 flex items-center justify-center">
                  ...
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0m-9 10h9"
                    />
                  </svg>
                </div>
                ...
                <div class="w-12 h-12 mx-auto mb-2 rounded-full bg-accent/10 flex items-center justify-center">
                  ...
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      d="M18.364 5.636l-3.536 3.536m1.414 1.414a9 9 0 01012.728 0M21 12a9 9 0 11-18 0 9 9 0 0118 0"
                    />
                  </svg>
                </div>
