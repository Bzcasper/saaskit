...
export class Validator {
  ...
  static validateType(value: unknown, type: string): boolean {
    switch (type) {
      case "string":
  ...
  createAPIKey: {
    ...
    permissions: {
      ...
      custom: (value: unknown[]) => {
        ...
        return value.every((p: string) => validPermissions.includes(p)) ||
          "Invalid permissions";
      },
    },
    rateLimitTier: {
...
export const SecurityValidators = {
  ...
  noSQLInjection: {
    custom: (value: string) => {
      ...
      return !dangerous.some((d) => value.toUpperCase().includes(d)) ||
