...
import { proxyHub, getProxyHealth } from "@/utils/proxy_hub.ts";
import { successResponse, errorResponse, toJson } from "@/utils/api_response.ts";
        ...
        const response = successResponse({
          ...
          configuration: {
            providers: [
              ...
              { name: "proxifly", priority: 1, type: "proxy", pricing: "affordable" },
              { name: "brightdata", priority: 2, type: "proxy", pricing: "premium" },
              { name: "smartproxy", priority: 3, type: "proxy", pricing: "budget" },
            ],
  ...
  async POST(req, ctx) {
      ...
      try {
        ...
        if (!provider || typeof enabled !== "boolean") {
          const response = errorResponse("INVALID_REQUEST", "Provider and enabled status required");
        ...
        if (!success) {
          const response = errorResponse("NOT_FOUND", `Provider '${provider}' not found`);
          return toJson(response, 404);
        }

        const response = successResponse({ provider, enabled }, `Provider ${provider} ${enabled ? "enabled" : "disabled"}`);
