...
import { proxyHub, fetchWithProxy } from "@/utils/proxy_hub.ts";
  ...
  async GET(req, ctx) {
      ...
      try {
        ...
        const response = successResponse(
          results,
          results.summary.successful > 0 ? "Proxy tests completed" : "All proxy tests failed",
        );
  ...
  async POST(req, ctx) {
      ...
      try {
            ...
            try {
              ...
              testResults.providers[instance] = {
                ...
                data: response.success
                  ? {
                      statusCode: response.statusCode,
                      hasAudioStreams: JSON.parse(response.body || "{}").audioStreams?.length > 0,
                    }
                  : undefined,
              };
            } catch (error) {
        ...
        if (testType === "lyrics") {
          ...
          try {
            const url = `https://lrclib.net/api/search?q=${encodeURIComponent("Never Gonna Give You Up Rick Astley")}`;
            const response = await proxyHub.fetch({
              url,
              timeout: 15000,
            });
        ...
        const response = successResponse(testResults, "Music API proxy tests completed");
        return toJson(response);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : "Unknown error";
        const response = errorResponse("INTERNAL_ERROR", errorMessage);
